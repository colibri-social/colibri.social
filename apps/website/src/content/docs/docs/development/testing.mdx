...
| AppView error-code parity | `tests/lexicons.rs` | `cargo test` |
...
wrote for nothing. See [Errors](/docs/development/errors) for what to do when you
add one.
...
| client | Preview release | publishes an installable client build to [pkg.pr.new](https://pkg.pr.new) |
