...
Provisions a new community: an account on the AppView's PDS, its four spaces, an owner role held by the caller, and a starter layout of two categories holding one text channel and one voice channel between them. The call can run long enough that a client should also watch `social.colibri.beta.sync.subscribeEvents` for progress.
