...
const MockVoiceCall: Component = () => {
  ...
  const phase1 = createAnimationTimeline(
    () => {
      ...
      return [
        ...
        { timestamp: 2500, execute: () => setVoiceGuests([firstGuest]) },
  ...
  function buildSoloCycle(currentGuestIds: string[]): BuildEventsResult {
    ...
    const convo2 = buildGroupConversation(joinTime + 500, [nextGuest.id, ME_ID], 4, setSpeakingId);
  ...
  function buildGroupCycle(currentGuestIds: string[]): BuildEventsResult {
    ...
    const threeWayIds = [...(currentGuest ? [currentGuest.id] : []), secondGuest.id, ME_ID];
    const convo2 = buildGroupConversation(secondJoinTime + 500, threeWayIds, 5, setSpeakingId);
  ...
  return (
    <div class="w-88 h-88 flex flex-col rounded-xl bg-background border border-border overflow-hidden select-none font-sans">
      ...
      <div class="flex flex-col py-4 px-3">
        <div class="flex items-center gap-2 px-4 pb-1.5 text-xs text-neutral-500 *:first:text-neutral-400 *:first:size-4 *:first:-translate-y-0.25">
          <ChevronDown />
      ...
      <div class="flex flex-col px-3">
        ...
        <div
          class="flex flex-col rounded-sm transition-all duration-300"
          classList={{ "bg-white/5": youJoined() }}
        >
          <div class="flex items-center gap-2 text-sm mx-3 px-1.5 py-0.75 rounded-sm transition-colors duration-300 mb-1.5">
            <SpeakerLow className="w-4 h-4 text-neutral-500 shrink-0" />
            <span
          ...
          <Show when={voiceGuests().length > 0 || youJoined()}>
            <div class="flex flex-col gap-1 pl-7 pr-3 pb-2">
              ...
              <Show when={youJoined()}>
                <VoiceUser name="You" color="bg-purple-500" speaking={speakingId() === ME_ID} />
              </Show>
            </div>
          </Show>
        </div>
...
const ChannelRow: Component<{
  ...
  <div
    class="flex items-center gap-2 text-sm mx-3 px-1.5 py-0.75 rounded-sm transition-colors duration-300 mb-1.5"
    classList={{
...
const VoiceUser: Component<{
  ...
  <div class="flex items-center gap-2 animate-in fade-in slide-in-from-bottom-1 duration-300">
    <div
      class={`size-5 rounded-full ${props.color} transition-shadow duration-150`}
      classList={{
        "shadow-[0_0_0_3px_var(--primary)]": props.speaking,
      }}
    />
