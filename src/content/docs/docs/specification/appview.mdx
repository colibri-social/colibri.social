---
title: AppView
---

import XRPCEndpointCard from "../../../../components/docs/XRPCEndpointCard.astro";

The Colibri AppView is responsible for aggregating message data from across the network and providing endpoints for interacting with this data. Any requests to the AppView are meant to be proxied via the user's PDS, to which the client application makes the requests. When requests are forwarded, the following DID URI should be used to indicate the target:

```
did:web:api.colibri.social#colibri_appview
```

## Implemented Endpoints

The AppView supports a range of XRPC methods that PDSs can call. All `com.atproto` endpoints adhere to the original specification. There are also certain `social.colibri` XRPC endpoints which allow for extended functionality. All of these endpoints are available under the `/xrpc/` path.

:::danger[Authentication]
If an endpoint requires authentication, the request needs to be forwarded by a PDS and carry a bearer token.
:::

### `com.atproto.identity.resolveDid`

Resolves a DID to a DID document. Does not bi-directionally verify the handle.

<XRPCEndpointCard
	method="GET"
	nsid="com.atproto.identity.resolveDid"
	queryParams={{
		did: {
			type: "did",
			required: true
		}
	}}
	response={{
		didDoc: {
			type: "DID Document",
			required: true
		}
	}}
/>

### `com.atproto.identity.resolveHandle`

Resolves an atproto handle (hostname) to a DID. Does not necessarily bi-directionally verify against the the DID document.

<XRPCEndpointCard
	method="GET"
	nsid="com.atproto.identity.resolveHandle"
	queryParams={{
		handle: {
			type: "handle",
			required: true
		}
	}}
	response={{
		did: {
			type: "did",
			required: true
		}
	}}
/>

### `com.atproto.identity.resolveIdentity`

Resolves an atproto handle (hostname) to a DID. Does not necessarily bi-directionally verify against the the DID document.

<XRPCEndpointCard
	method="GET"
	nsid="com.atproto.identity.resolveIdentity"
	queryParams={{
		identifier: {
			type: "did | handle",
			required: true
		}
	}}
	response={{
		did: {
			type: "did",
			required: true
		},
		handle: {
			type: "handle",
			required: true
		},
		didDoc: {
			type: "DID Document",
			required: true
		}
	}}
/>

### `com.atproto.repo.getRecord`

Get a single cached record from the AppView.

<XRPCEndpointCard
	method="GET"
	nsid="com.atproto.repo.getRecord"
	queryParams={{
		repo: {
			type: "at-identifier",
			required: true
		},
		collection: {
			type: "nsid",
			required: true
		},
		rkey: {
			type: "record-key",
			required: true
		},
		cid: {
			type: "cid",
			required: false
		}
	}}
	response={{
		uri: {
			type: "at-uri",
			required: true
		},
		value: {
			type: "object",
			required: true
		},
		cid: {
			type: "cid",
			required: false
		}
	}}
/>

### `com.atproto.repo.listRecords`

List a range of cached records in a repository, matching a specific collection.

<XRPCEndpointCard
	method="GET"
	nsid="com.atproto.repo.listRecords"
	queryParams={{
		repo: {
			type: "at-identifier",
			required: true
		},
		collection: {
			type: "nsid",
			required: true
		},
		limit: {
			type: "integer",
			required: false,
		},
		cursor: {
			type: "string",
			required: false,
		},
		reverse: {
			type: "boolean",
			required: false,
		}
	}}
	response={{
		cursor: {
			type: "string",
			required: false,
		},
		records: {
			type: "object[]",
			required: true
		}
	}}
/>

---

### `social.colibri.actor.getData`

Get the status data and bluesky profile for a Colibri user.

<XRPCEndpointCard
	method="GET"
	nsid="social.colibri.actor.getData"
	queryParams={{
		itendifier: {
			type: "did | handle",
			required: true
		}
	}}
	response={{
		did: {
			type: "did",
			required: true,
		},
		handle: {
			type: "handle",
			required: true,
		},
		data: {
			type: "object",
			required: true,
			children: {
				displayName: {
					type: "string",
					required: true,
				},
				avatar: {
					type: "blob",
					required: false,
				},
				banner: {
					type: "blob",
					required: false,
				},
				description: {
					type: "string",
					required: false,
				},
				onlineState: {
					type: "string",
					required: true,
				},
				status: {
					type: "object",
					required: false,
					children: {
						emoji: {
							type: "string",
							required: false,
						},
						text: {
							type: "string",
							required: true
						}
					}
				},
			}
		}
	}}
/>

### `social.colibri.actor.setState`

Sets the online state for a Colibri user.

<XRPCEndpointCard
	method="POST"
	nsid="social.colibri.actor.setState"
	authenticated
	body={{
		state: {
			type: "string",
			required: true
		}
	}}
	response={{
		state: {
			type: "string",
			required: true
		}
	}}
/>

### `social.colibri.actor.listCommunities`

Lists all communities the Colibri user is a part of (and not blocked from).

<XRPCEndpointCard
	method="GET"
	nsid="social.colibri.actor.listCommunities"
	authenticated
	response={{
		communities: {
			type: "object[]",
			required: true,
			children: {
				name: {
					type: "string",
					required: true
				},
				picture: {
					type: "blob",
					required: false
				},
				description: {
					type: "string",
					required: false
				},
				categoryOrder: {
					type: "string[]",
					required: false
				},
				uri: {
					type: "at-uri",
					required: false
				},
				requiresApprovalToJoin: {
					type: "boolean",
					required: true
				},
			}
		}
	}}
/>

### `social.colibri.sync.subscribeEvents`

Opens a connection with a WebSocket stream that transmits relevant events for the user.

<XRPCEndpointCard
	method="WS"
	nsid="social.colibri.sync.subscribeEvents"
	authenticated
/>

#### Events

The event stream can choose to transmit events to clients who have signed up to receive them.

##### `ack`

Sent when the AppView receives a [`heartbeat`](#heartbeat) message from the client.

##### `community_event`

Events of this type are sent to clients when a community has been updated or deleted.

##### `member_event`

Events of this type are sent to clients when a member has joined or left (or been blocked from) a community.

##### `category_event`

Events of this type are sent to clients when a category has been created, updated, or deleted.

##### `channel_event`

Events of this type are sent to clients when a channel has been created, updated, or deleted.

##### `message_event`

Events of this type are sent to clients when a message has been sent, edited, or deleted.

##### `reaction_event`

Events of this type are sent to clients when a reaction has been added to or removed from a message.

##### `user_event`

Events of this type are sent to clients when a user the client knows about has updated their Bluesky profile or Colibri status.

#### Messages

The following messages can be sent from the client to the AppView

##### `heartbeat`

A generic event that carries no data. It is used to keep the socket connection alive. The AppView will always respond with an `ack` event.

##### `subscribe`

Clients can send a `subscribe` message to the AppView, supplying a list of communities for which they wish to receive updates. The AppView handles the rest and infers which users, channels, etc. the client needs to know about.

##### `unsubscribe`

Clients can send an `unsubscribe` message to the AppView, supplying a list of communities, for which they wish to no longer receive events.

##### `activity`

A message used to prevent clients from having their online status set to "Away" after a certain amount of time. Clients should send this message periodically while the user actively interacts with the app.

##### `voice_join`

Clients must send a `voice_join` message to the AppView when the user joins a voice channel.

##### `voice_leave`

Clients must send a `voice_leave` message to the AppView when the user leaves a voice channel.

##### `state`

A message used to inform the AppView about the new preferred online state of the user.

### `social.colibri.community.listBlockedUsers`

Lists all blocked users for a community.

<XRPCEndpointCard
	method="GET"
	nsid="social.colibri.community.listBlockedUsers"
	body={{
		'community': {
			type: "at-uri",
			required: true
		}
	}}
	response={{
		dids: {
			type: "did[]",
			required: true,
		}
	}}
/>

### `social.colibri.community.listCategories`

Lists all categories in a community.

<XRPCEndpointCard
	method="GET"
	nsid="social.colibri.community.listCategories"
	body={{
		'community': {
			type: "at-uri",
			required: true
		}
	}}
	response={{
		categories: {
			type: "object[]",
			required: true,
			children: {
				uri: {
					type: "at-uri",
					required: true,
				},
				name: {
					type: "string",
					required: true,
				},
				channelOrder: {
					type: "at-uri[]",
					required: true,
				},
			}
		}
	}}
/>

### `social.colibri.community.listChannels`

Lists all channels in a community.

<XRPCEndpointCard
	method="GET"
	nsid="social.colibri.community.listChannels"
	body={{
		'community': {
			type: "at-uri",
			required: true
		}
	}}
	response={{
		channels: {
			type: "object[]",
			required: true,
			children: {
				uri: {
					type: "at-uri",
					required: true,
				},
				name: {
					type: "string",
					required: true,
				},
				type: {
					type: "string",
					required: true,
				},
				category: {
					type: "at-uri",
					required: true,
				},
			}
		}
	}}
/>

### `social.colibri.community.listMembers`

Lists all members with their roles, status information and Bluesky profiles.

<XRPCEndpointCard
	method="GET"
	nsid="social.colibri.community.listMembers"
	body={{
		'community': {
			type: "at-uri",
			required: true
		}
	}}
	response={{
		'members': {
			type: "object[]",
			required: true,
			children: {
				did: {
					type: "did",
					required: true,
				},
				handle: {
					type: "handle",
					required: true,
				},
				data: {
					type: "object",
					required: true,
					children: {
						displayName: {
							type: "string",
							required: true,
						},
						avatar: {
							type: "blob",
							required: false,
						},
						banner: {
							type: "blob",
							required: false,
						},
						description: {
							type: "string",
							required: false,
						},
						onlineState: {
							type: "string",
							required: true,
						},
						status: {
							type: "object",
							required: false,
							children: {
								emoji: {
									type: "string",
									required: false,
								},
								text: {
									type: "string",
									required: true
								}
							}
						},
					}
				}
			}
		}
	}}
/>

### `social.colibri.community.blockMessage`

Block a message in a given community.

<XRPCEndpointCard
	method="POST"
	nsid="social.colibri.community.blockMessage"
	authenticated
	body={{
		'community': {
			type: "at-uri",
			required: true
		},
		'message': {
			type: "at-uri",
			required: true
		},
	}}
	response={{
		'message': {
			type: "at-uri",
			required: true,
		}
	}}
/>

### `social.colibri.community.blockUser`

Block a user from writing messages in a given community.

<XRPCEndpointCard
	method="POST"
	nsid="social.colibri.community.blockUser"
	authenticated
	body={{
		'community': {
			type: "at-uri",
			required: true
		},
		'identifier': {
			type: "did | handle",
			required: true
		}
	}}
	response={{
		'did': {
			type: "did",
			required: true,
		},
		'handle': {
			type: "handle",
			required: true,
		}
	}}
/>

### `social.colibri.community.unblockUser`

Unblock a user from a community, allowing them to write messages there again.

<XRPCEndpointCard
	method="POST"
	nsid="social.colibri.community.unblockUser"
	authenticated
	body={{
		'community': {
			type: "at-uri",
			required: true
		},
		'identifier': {
			type: "did | handle",
			required: true
		}
	}}
	response={{
		'did': {
			type: "did",
			required: true,
		},
		'handle': {
			type: "handle",
			required: true,
		}
	}}
/>

### `social.colibri.community.createInvitation`

Create an invitation for the specified community.

<XRPCEndpointCard
	method="POST"
	nsid="social.colibri.community.createInvitation"
	authenticated
	body={{
		'community': {
			type: "at-uri",
			required: true
		},
	}}
	response={{
		'code': {
			type: "string",
			required: true,
		},
		'community': {
			type: "at-uri",
			required: true,
		},
		'createdBy': {
			type: "did",
			required: true
		},
		'active': {
			type: "boolean",
			required: true
		}
	}}
/>

### `social.colibri.community.resolveInvitation`

Get information about a given invitation code.

<XRPCEndpointCard
	method="GET"
	nsid="social.colibri.community.getInvitation"
	authenticated
	body={{
		'code': {
			type: "string",
			required: true
		},
	}}
	response={{
		'code': {
			type: "string",
			required: true,
		},
		'community': {
			type: "at-uri",
			required: true,
		},
		'createdBy': {
			type: "did",
			required: true
		},
		'active': {
			type: "boolean",
			required: true
		}
	}}
/>

### `social.colibri.community.listInvitations`

List all invitations for the specified community.

<XRPCEndpointCard
	method="POST"
	nsid="social.colibri.community.listInvitations"
	authenticated
	body={{
		'uri': {
			type: "at-uri",
			required: true
		},
	}}
	response={{
		codes: {
			type: "object[]",
			required: true,
			children: {
				'code': {
					type: "string",
					required: true,
				},
				'community': {
					type: "at-uri",
					required: true,
				},
				'createdBy': {
					type: "did",
					required: true
				},
				'active': {
					type: "boolean",
					required: true
				}
			}
		}
	}}
/>

### `social.colibri.community.deleteInvitation`

Delete an invitation for the specified community.

<XRPCEndpointCard
	method="POST"
	nsid="social.colibri.community.deleteInvitation"
	authenticated
	body={{
		'uri': {
			type: "at-uri",
			required: true
		},
		'code': {
			type: "string",
			required: true
		},
	}}
	response={{
		'code': {
			type: "string",
			required: true,
		}
	}}
/>

### `social.colibri.channel.listMessages`

Get a paginated message history for the specified channel, newest messages first.

<XRPCEndpointCard
	method="GET"
	nsid="social.colibri.channel.listMessages"
	body={{
		'channel': {
			type: "at-uri",
			required: true
		},
		'limit': {
			type: "integer",
			required: false
		},
		'cursor': {
			type: "string",
			required: false
		},
		'all': {
			type: "boolean",
			required: false
		},
	}}
	response={{
		'messages': {
			type: "object[]",
			required: true,
			children: {
				uri: {
					type: "at-uri",
					required: true,
				},
				text: {
					type: "string",
					required: true,
				},
				facets: {
					type: "Facet[]",
					required: true,
				},
				channel: {
					type: "at-uri",
					required: true,
				},
				community: {
					type: "at-uri",
					required: true,
				},
				author: {
					type: "did",
					required: true,
				},
				parent: {
					type: "at-uri",
					required: true,
				},
				attachments: {
					type: "blob[]",
					required: true,
				},
			}
		}
	}}
/>

### `social.colibri.channel.getReadCursor`

Get the read cursor for the current user and specified channel.

<XRPCEndpointCard
	method="GET"
	nsid="social.colibri.channel.getReadCursor"
	authenticated
	body={{
		'channel': {
			type: "at-uri",
			required: true
		},
	}}
	response={{
		'uri': {
			type: "at-uri",
			required: true,
		},
		'cursor': {
			type: "string",
			required: true,
		},
		'channel': {
			type: "at-uri",
			required: true,
		}
	}}
/>

### `social.colibri.message.listReactions`

Get all reactions for a specified message, grouped by emoji with reactor DIDs.

<XRPCEndpointCard
	method="GET"
	nsid="social.colibri.channel.listReactions"
	body={{
		'message': {
			type: "at-uri",
			required: true
		},
	}}
	response={{
		reactions: {
			type: "object[]",
			required: true,
			children: {
				emoji: {
					type: "string",
					required: true,
				},
				count: {
					type: "integer",
					required: true,
				},
				reactorDIDs: {
					type: "string[]",
					required: true,
				},
			}
		}
	}}
/>

## Humming

The Colibri infrastructure is designed to be easy to self-host. In cases where users prefer to host their own infrastructure, under normal circumstances, data that is not saved on the protocol (like online/offline status, writing presence, etc.) is not able to be transmitted to other users who use a different app view.

The Colibri AppView supports a system called **Humming** to curcumvent this. Other AppView instances that wish to share off-protocol status data with users on other instances can share them via a **Hum**, a message sent to another AppView instance's `social.colibri.sync.sendHum` endpoint, which can then choose to relay it to its clients via its [`social.colibri.sync.subscribeEvents`](#socialcolibrisyncsubscribeevents) WebSocket stream. Additionally, AppViews that wish to forward these events to other AppViews can expose a `social.colibri.sync.subscribeHums` endpoint, to which other AppViews can subscribe.

### `social.colibri.sync.sendHum`

Inform another AppView of an event that has occured.

<XRPCEndpointCard
	method="POST"
	nsid="social.colibri.sync.sendHum"
	body={{
		'event': {
			type: "object",
			required: true
		}
	}}
/>

### `social.colibri.sync.subscribeHums`

<XRPCEndpointCard
	method="WS"
	nsid="social.colibri.sync.subscribeHums"
/>

For transmitted event types, please refer to [`social.colibri.sync.subscribeEvents`](#socialcolibrisyncsubscribeevents).

:::note
It is not possible to send messages to an AppView via the `social.colibri.sync.subscribeHums` endpoint. It only functions as an egress.
:::
